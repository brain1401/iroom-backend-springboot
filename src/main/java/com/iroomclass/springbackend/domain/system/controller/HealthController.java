...
public class HealthController {
     ...
     * 헬스 체크 엔드포인트
     */
    @Operation(summary = "종합 시스템 헬스체크", description = "Spring Boot 애플리케이션, 데이터베이스, AI 서버의 상태를 종합적으로 확인합니다.", responses = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "헬스체크 성공", content = @Content(schema = @Schema(implementation = ApiResponse.SuccessResponse.class), examples = @ExampleObject(name = "헬스체크 성공", summary = "정상 응답 예시", value = """
                    {
                      "result": "SUCCESS",
    ...
    public ApiResponse<SystemHealthResponse> healthCheck() {
        SystemHealthResponse healthStatus = systemService.checkHealth();
