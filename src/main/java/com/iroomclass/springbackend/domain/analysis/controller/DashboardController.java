...
public class DashboardController {
     ...
     */
    @Operation(summary = "전체 학년 통합 통계 조회", description = "모든 학년의 통합 통계 정보를 조회합니다", responses = {
     ...
     */
    @Operation(summary = "학년별 시험 제출 현황 조회", description = "특정 학년의 시험별 제출 현황을 조회합니다", responses = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiResponse.SuccessResponse.class), examples = @ExampleObject(name = "잘못된 학년", summary = "학년 값이 올바르지 않음", value = """
                    ...
                    """))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "오류", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiResponse.ErrorResponse.class), examples = @ExampleObject(name = "서버 오류", summary = "서버 내부 오류 발생", value = """
                    {
                      "result": "ERROR",
                      "message": "서버 내부 오류가 발생했습니다",
                      "data": null
    ...
    public ApiResponse<GradeSubmissionStatusResponse> getGradeSubmissionStatus(
            @Parameter(description = "학년", example = "1") @PathVariable Integer grade) {
     ...
     */
    @Operation(summary = "학년별 성적 분포도 조회", description = "특정 학년의 성적 분포도를 조회합니다", responses = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiResponse.SuccessResponse.class), examples = @ExampleObject(name = "잘못된 학년", summary = "학년 값이 올바르지 않음", value = """
                    {
                      "result": "ERROR",
                      "message": "파라미터 'grade'의 값이 올바르지 않습니다",
