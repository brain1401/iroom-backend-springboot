 ...
 * 
 * <p>
 * 문제지 조회, 검색, 통계 관련 API를 제공합니다.
 * 선생님이 생성한 문제지들을 효율적으로 관리할 수 있도록 지원합니다.
 * </p>
 */
@RestController
...
public class ExamSheetController {
     ...
     * 시험지 생성
     */
    @Operation(summary = "시험지 생성", description = """
            새로운 시험지를 생성합니다.

            생성 과정:
            1. 문제 ID들의 존재 여부 및 학년 일치성 검증
            2. 문제 순서 연속성 검증 (1, 2, 3, ...)
            3. 시험지명 중복 검증 (같은 학년 내)
            4. 시험지 및 문제 매핑 정보 저장
            ...
            - 같은 학년에서 시험지명 중복 불가
            """, responses = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "201", description = "생성 성공", content = @Content(schema = @Schema(implementation = ExamSheetDto.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "잘못된 요청 (검증 실패)", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "시험지명 중복", content = @Content(schema = @Schema(implementation = ApiResponse.class)))
    })
    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ResponseEntity<?> createExamSheet(
            @Valid @RequestBody CreateExamSheetRequest request) {
        log.info("시험지 생성 요청: examName={}, grade={}, questionCount={}", 
                request.examName(), request.grade(), request.questions().size());

        try {
            ExamSheetDto examSheet = examSheetService.createExamSheet(request);
            log.info("시험지 생성 성공: examSheetId={}, examName={}", examSheet.id(), examSheet.examName());

            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(ApiResponse.success("시험지 생성 성공", examSheet));
        } catch (IllegalArgumentException e) {
            log.warn("시험지 생성 실패 (검증 오류): examName={}, error={}", request.examName(), e.getMessage());
            return ResponseEntity.badRequest()
                    .body(ApiResponse.error("검증 실패: " + e.getMessage()));
        } catch (RuntimeException e) {
            log.error("시험지 생성 실패: examName={}, error={}", request.examName(), e.getMessage(), e);
            throw e;
        }
    }
     ...
     * 문제지 상세 조회
     */
    @Operation(summary = "문제지 상세 조회", description = """
            ...
            - 문제별 배점 정보
            """, responses = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공", content = @Content(schema = @Schema(implementation = ExamSheetDto.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "문제지를 찾을 수 없음", content = @Content(schema = @Schema(implementation = ApiResponse.class)))
    })
    @GetMapping("/{examSheetId}")
    public ResponseEntity<ApiResponse<ExamSheetDto>> getExamSheet(
            @Parameter(description = "문제지 고유 식별자", required = true) @PathVariable UUID examSheetId) {
     ...
     */
    @Operation(summary = "문제지 목록 조회", description = """
    ...
    public ResponseEntity<ApiResponse<Page<ExamSheetDto>>> getExamSheets(
            ...
            @Parameter(description = "정렬 기준 필드", example = "createdAt") @RequestParam(defaultValue = "createdAt") String sort,
            ...
            @Parameter(description = "검색 키워드 (문제지 이름)", example = "수학") @RequestParam(required = false) String search) {
     ...
     * 학년별 문제지 통계
     */
    @Operation(summary = "학년별 문제지 통계", description = """
     ...
     * 최근 생성된 문제지 목록
     */
    @Operation(summary = "최근 생성된 문제지 목록", description = """
     ...
     * 문제지별 사용 현황 (시험 발행 횟수)
     */
    @Operation(summary = "문제지별 사용 현황", description = """
            특정 문제지가 실제 시험으로 발행된 횟수를 조회합니다.
            문제지의 활용도를 파악할 수 있습니다.
            """, responses = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "문제지를 찾을 수 없음")
    })
