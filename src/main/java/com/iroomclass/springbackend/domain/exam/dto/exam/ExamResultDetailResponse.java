...
public record ExamResultDetailResponse(
        ...
        @Schema(description = "객관식 문제 수", example = "15") int multipleChoiceCount,

        @Schema(description = "주관식 문제 수", example = "5") int subjectiveCount,

        @Schema(description = "단원명 목록", example = "정수와 유리수, 문자와 식, 일차방정식") String unitNames,

        @Schema(description = "정답 수", example = "17") int correctCount,

        @Schema(description = "오답 수", example = "3") int incorrectCount,

        @Schema(description = "문제별 결과 목록") List<QuestionResult> questionResults) {
    /**
    ...
    public record QuestionResult(
            @Schema(description = "문제 ID", example = "123e4567-e89b-12d3-a456-426614174002") UUID questionId,
            ...
            @Schema(description = "학생 답안", example = "42") String studentAnswer,

            @Schema(description = "정답", example = "42") String correctAnswer) {
        /**
         * Compact Constructor - 입력 검증 수행
         */
