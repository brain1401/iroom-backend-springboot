 ...
 * 
 * <p>
 * AI 서버의 텍스트 인식 API를 프록시하는 컨트롤러입니다.
 * 실제 처리는 AI 서버에서 수행하고, Spring Boot는 단순히 요청을 전달합니다.
 * </p>
 */
@RestController
...
public class AiTextRecognitionController {
    ...
    @Operation(summary = "답안지 텍스트 인식", description = "업로드된 답안지 이미지에서 텍스트를 인식하여 구조화된 데이터로 반환합니다", responses = {
            @ApiResponse(responseCode = "200", description = "인식 성공", content = @Content(schema = @Schema(implementation = AiServerResponse.class))),
    ...
    public Mono<ResponseEntity<AiServerResponse>> recognizeAnswerSheet(
            @Parameter(description = "답안지 이미지 파일", required = true) @RequestPart("file") MultipartFile file) {
    ...
    @Operation(summary = "배치 텍스트 인식", description = "여러 이미지 파일을 한번에 처리하여 텍스트를 인식합니다", responses = {
            @ApiResponse(responseCode = "200", description = "배치 처리 성공"),
            @ApiResponse(responseCode = "400", description = "잘못된 요청"),
            @ApiResponse(responseCode = "500", description = "AI 서버 오류")
    })
    public Mono<ResponseEntity<Object>> recognizeBatch(
            @Parameter(description = "인식할 이미지 파일들", required = true) @RequestPart("files") MultipartFile[] files) {
     ...
     */
    @GetMapping("/metrics")
    @Operation(summary = "인식 메트릭 조회", description = "AI 서버의 텍스트 인식 성능 메트릭을 조회합니다", responses = {
    ...
    @Operation(summary = "대시보드 정보 조회", description = "AI 서버의 텍스트 인식 대시보드 데이터를 조회합니다", responses = {
    ...
    @Operation(summary = "캐시 무효화", description = "AI 서버의 텍스트 인식 관련 캐시를 무효화합니다", responses = {
     ...
     */
    @GetMapping("/health")
    @Operation(summary = "AI 서버 상태 확인", description = "AI 서버의 텍스트 인식 서비스 상태를 확인합니다", responses = {
