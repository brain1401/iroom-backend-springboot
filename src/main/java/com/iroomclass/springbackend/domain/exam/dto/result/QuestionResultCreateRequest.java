...
public record QuestionResultCreateRequest(
        @Schema(description = "문제 ID", example = "123e4567-e89b-12d3-a456-426614174005", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "문제 ID는 필수입니다")
        UUID questionId,

        @Schema(description = "답안지 ID", example = "123e4567-e89b-12d3-a456-426614174004", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "답안지 ID는 필수입니다")
        UUID answerSheetId,

        @Schema(description = "정답 여부", example = "true", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "정답 여부는 필수입니다")
        Boolean isCorrect,

        @Schema(description = "획득 점수", example = "5", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "획득 점수는 필수입니다")
        @Min(value = 0, message = "점수는 0 이상이어야 합니다")
        @Max(value = 100, message = "점수는 100 이하여야 합니다")
        Integer score,

        @Schema(description = "만점", example = "5", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "만점은 필수입니다")
        @Min(value = 1, message = "만점은 1 이상이어야 합니다")
        @Max(value = 100, message = "만점은 100 이하여야 합니다")
        Integer maxScore,

        @Schema(description = "채점 방법", example = "AUTO", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "채점 방법은 필수입니다")
        ...
        String aiAnalysis) implements BaseRecord {
    ...
    public QuestionResultCreateRequest {
        ...
        Objects.requireNonNull(maxScore, "만점은 필수입니다");
        Objects.requireNonNull(gradingMethod, "채점 방법은 필수입니다");
        
        if (score < 0 || score > maxScore) {
            throw new IllegalArgumentException("점수는 0 이상 만점 이하여야 합니다");
        }
        if (maxScore <= 0) {
            throw new IllegalArgumentException("만점은 1 이상이어야 합니다");
        }
