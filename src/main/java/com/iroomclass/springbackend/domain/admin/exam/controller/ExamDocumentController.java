...
public class ExamDocumentController {
    ...
    @Operation(
        summary = "시험지 문서 생성",
        description = "시험지 초안을 기반으로 답안지, 문제지, 답안을 생성합니다. 기존 문서가 있다면 재생성됩니다."
    )
    @ApiResponses({
    ...
    public ResponseEntity<ExamDocumentCreateResponse> createExamDocuments(@RequestBody ExamDocumentCreateRequest request) {
        ...
        log.info("시험지 문서 생성 성공: 시험지 초안 ID={}, 문서={}개", 
            response.getExamDraftId(), response.getDocumentCount());
    ...
    @Operation(
        summary = "시험지 초안별 문서 목록 조회",
        description = "특정 시험지 초안의 모든 문서 목록을 조회합니다. 답안지, 문제지, 답안을 포함합니다."
    )
    @ApiResponses({
    ...
    public ResponseEntity<ExamDocumentListResponse> getExamDocumentsByDraft(
        ...
        log.info("시험지 초안 {} 문서 목록 조회 성공: {}개", examDraftId, response.getTotalCount());
    ...
    @Operation(
        summary = "시험지 문서 상세 조회",
        description = "특정 시험지 문서의 상세 정보를 조회합니다. HTML 형태의 문서 내용을 포함합니다."
    )
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "조회 성공"),
        @ApiResponse(responseCode = "400", description = "잘못된 문서 ID"),
        @ApiResponse(responseCode = "404", description = "존재하지 않는 시험지 문서")
    })
    public ResponseEntity<ExamDocumentDetailResponse> getExamDocumentDetail(
        ...
        log.info("시험지 문서 {} 상세 조회 성공: 타입={}", documentId, response.getDocumentTypeName());
    ...
    @Operation(
        summary = "시험지 문서 삭제",
        description = "시험지 초안과 연관된 모든 문서(답안지, 문제지, 답안)를 삭제합니다. 시험지 목록에서 제거됩니다."
    )
    @ApiResponses({
    ...
    public ResponseEntity<String> deleteExamDocuments(
            @Parameter(description = "시험지 초안 ID", example = "1") 
            @PathVariable Long examDraftId) {
