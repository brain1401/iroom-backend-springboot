...
public class StatisticsController {
    ...
    @Operation(
        summary = "학년별 통계 조회",
        description = "해당 학년의 최근 시험 평균 점수와 오답률 높은 세부 단원 통계를 조회합니다."
    )
    @ApiResponses({
        @ApiResponse(
            responseCode = "200", 
            description = "통계 조회 성공",
            content = @Content(schema = @Schema(implementation = GradeStatisticsResponse.class))
        ),
        @ApiResponse(responseCode = "400", description = "잘못된 학년 값"),
        @ApiResponse(responseCode = "404", description = "해당 학년의 데이터가 없음")
    })
